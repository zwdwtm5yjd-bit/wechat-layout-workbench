...
compose exec -T api node -e \
  "Promise.all(['/health/live','/health/ready','/api/openapi.json'].map(async(path)=>{const response=await fetch('http://127.0.0.1:3001'+path);if(!response.ok)throw new Error(path+' returned '+response.status);const body=await response.json();if(path.startsWith('/health/')&&body.status!=='ok')throw new Error(path+' is not ok');if(path==='/health/ready'&&(body.info?.database?.status!=='up'||body.info?.redis?.status!=='up'||body.info?.objectStorage?.status!=='up'||body.info?.worker?.status!=='up'||body.info?.api?.registeredDependencyChecks!==4))throw new Error('database, Redis, object storage or worker readiness probe is unavailable');if(path==='/api/openapi.json'&&(!body.openapi||!body.paths?.['/api/v1/auth/login']||!body.paths?.['/api/v1/auth/me']||!body.paths?.['/api/v1/jobs']?.get||!body.paths?.['/api/v1/jobs/{jobId}']?.get||!body.paths?.['/api/v1/jobs/{jobId}/cancel']?.post||!body.paths?.['/api/v1/jobs/{jobId}/retry']?.post||!body.paths?.['/api/v1/jobs/{jobId}/events']?.get||!body.paths?.['/api/v1/themes']?.get||!body.paths?.['/api/v1/themes/{themeId}']?.get||!body.paths?.['/api/v1/articles/{articleId}/themes/{themeId}/preview']?.post||!body.paths?.['/api/v1/articles/{articleId}/themes/{themeId}/apply']?.post||!body.paths?.['/api/v1/articles']||!body.paths?.['/api/v1/articles/{articleId}/duplicate']||!body.paths?.['/api/v1/articles/{articleId}/document']?.put||!body.paths?.['/api/v1/articles/{articleId}/snapshots']?.post||!body.paths?.['/api/v1/articles/{articleId}/snapshots/{snapshotId}/restore']?.post||!body.paths?.['/api/v1/articles/{articleId}/render-wechat']?.post||!body.paths?.['/api/v1/articles/{articleId}/copy-payload']?.post||!body.paths?.['/api/v1/articles/{articleId}/copy-records']?.post||!body.paths?.['/api/v1/imports/paste']?.post||!body.paths?.['/api/v1/imports/{articleId}/structure']?.get||!body.paths?.['/api/v1/imports/{articleId}/structure']?.put||!body.paths?.['/api/v1/resources']?.get||!body.paths?.['/api/v1/resources/uploads']?.post||!body.paths?.['/api/v1/resources/uploads/{uploadId}/complete']?.post||!body.paths?.['/api/v1/resources/{resourceId}/access-url']?.post||!body.paths?.['/api/v1/resources/{resourceId}/references']?.get))throw new Error('OpenAPI authentication, job, theme, content, copy or resource contract is invalid')})).then(()=>process.exit(0)).catch((error)=>{console.error(error.message);process.exit(1)})"
...
echo "4/10 验证任务中心、资源直传、粘贴导入、文章 CRUD、文档并发锁与不可变快照"
compose exec -T api node --input-type=module < "$script_dir/job-smoke.mjs"
...
assert_healthy web
compose exec -T api node -e \
  "async function run(){for(let attempt=0;attempt<60;attempt+=1){const response=await fetch('http://127.0.0.1:3001/health/ready');if(response.ok&&(await response.json()).info?.worker?.status==='up')return;await new Promise((resolve)=>setTimeout(resolve,500))}throw new Error('API and worker did not recover after Redis restart')}run().then(()=>process.exit(0)).catch((error)=>{console.error(error.message);process.exit(1)})"
