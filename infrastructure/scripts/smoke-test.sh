...
compose exec -T api node -e \
  "Promise.all(['/health/live','/health/ready','/api/openapi.json'].map(async(path)=>{const response=await fetch('http://127.0.0.1:3001'+path);if(!response.ok)throw new Error(path+' returned '+response.status);const body=await response.json();if(path.startsWith('/health/')&&body.status!=='ok')throw new Error(path+' is not ok');if(path==='/health/ready'&&(body.info?.database?.status!=='up'||body.info?.redis?.status!=='up'||body.info?.api?.registeredDependencyChecks!==2))throw new Error('database or Redis readiness probe is unavailable');if(path==='/api/openapi.json'&&(!body.openapi||!body.paths?.['/api/v1/auth/login']||!body.paths?.['/api/v1/auth/me']||!body.paths?.['/api/v1/articles']||!body.paths?.['/api/v1/articles/{articleId}/duplicate']||!body.paths?.['/api/v1/articles/{articleId}/document']?.put||!body.paths?.['/api/v1/articles/{articleId}/snapshots']?.post||!body.paths?.['/api/v1/articles/{articleId}/snapshots/{snapshotId}/restore']?.post))throw new Error('OpenAPI authentication, article, document or snapshot contract is invalid')})).then(()=>process.exit(0)).catch((error)=>{console.error(error.message);process.exit(1)})"
