...
function visualDefinition(input: {
  readonly assetKind: "png" | "svg";
  readonly assetPath: string;
  readonly body: string;
  readonly category: ComponentCategory;
  readonly categoryLabel: "SVG装饰" | "图集模块" | "高级模块";
  readonly componentId: string;
  readonly description: string;
  readonly eyebrow: string;
  readonly imageCount?: 2 | 3;
  readonly name: string;
  readonly scenarios: readonly string[];
  readonly title: string;
  readonly tokenMap: ComponentManifestV1_1["defaultTokenMap"];
  ...
  const imageSlots = Array.from({ length: imageCount }, (_, index) =>
    slot(`image${String(index + 1)}`, `图片 ${String(index + 1)}`, "image", "content"),
  );
  const imageDefaults = Object.fromEntries(
    imageSlots.map((imageSlot, index) => [
      imageSlot.slotId,
      {
        alt: `${input.name}图片 ${String(index + 1)}`,
        resourceId: `component_slot_visual_image_${String(index + 1)}_pending`,
        ...(index === 0 ? { caption: "替换为你的图片" } : {}),
      },
    ]),
  );
  return {
    ...
    insertionPreset: {
      ...
      slotBindings: [
        ...
        ...imageSlots.map((imageSlot, index) => ({
          slotId: imageSlot.slotId,
          target: { index: index + 1, kind: "child_image" as const },
        })),
      ],
    },
    name: input.name,
    ...
    semanticRoles: [
      ...
      ...(input.category === "GALLERY" ? ["gallery"] : []),
    ],
    slots: [
      slot("eyebrow", "眉题", "text", "eyebrow", {
        maxLength: 40,
        recommendedMaxLength: 12,
      }),
      slot("title", "标题", "text", "title", {
        maxLength: 120,
        recommendedMaxLength: 28,
      }),
      slot("body", "正文", "rich_text", "content", {
        maxLength: 4_000,
...
const definitions: readonly OfficialComponentDefinition[] = [
  ...
  visualDefinition({
    assetKind: "png",
    assetPath: "/visual-assets/ink-mountain-panorama.png",
    body: "远山含黛，流水无声。让正文在留白与层次之间自然展开。",
    category: "HERO",
    categoryLabel: "高级模块",
    componentId: "cmp_hero_ink_mountain_001",
    description: "原创水墨 PNG 主视觉，组合远山、朱日、留白与居中标题。",
    eyebrow: "ORIENTAL STORY",
    name: "水墨远山主视觉",
    scenarios: ["culture", "festival", "solar_term", "longform"],
    title: "山水有清音",
    tokenMap: {
      backgroundColor: "{colors.background}",
      borderColor: "{colors.border}",
  ...
  visualDefinition({
    assetKind: "png",
    assetPath: "/visual-assets/autumn-persimmon-branch.png",
    body: "风吹一片叶，万物已惊秋。这里适合放置节气释义、活动导语与温馨提醒。",
    category: "INTRO",
    categoryLabel: "高级模块",
    componentId: "cmp_intro_autumn_persimmon_001",
    description: "原创柿子枝水彩 PNG 与左侧文字区组合，适合秋季节气和东方生活方式内容。",
    eyebrow: "AUTUMN · 二十四节气",
    name: "秋柿节气导语",
    scenarios: ["solar_term", "autumn", "culture", "lifestyle"],
    title: "立秋 · 天地始肃",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.accent}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/mist-mountains.svg",
    body: "以青绿远山承托章节开场，正文区域保持克制和舒展。",
    category: "SVG",
    categoryLabel: "SVG装饰",
    componentId: "cmp_head_mist_mountains_007",
    description: "原创青绿水墨 SVG 题头，适合节气、人文、诗词与文旅专题。",
    eyebrow: "CHAPTER 01",
    name: "青绿远山题头",
    scenarios: ["culture", "travel", "solar_term", "poetry"],
    title: "行到水穷处，坐看云起时",
    tokenMap: {
      backgroundColor: "{colors.background}",
      color: "{colors.primaryDark}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/cloud-scroll.svg",
    body: "金线祥云只承担分层与收束，让标题保持东方仪式感而不过度喧闹。",
    category: "SVG",
    categoryLabel: "SVG装饰",
    componentId: "cmp_head_cloud_scroll_008",
    description: "原创金线祥云 SVG 章节题头，适合传统节日、非遗与品牌文化内容。",
    eyebrow: "东方美学",
    name: "金线祥云题头",
    scenarios: ["culture", "heritage", "festival", "brand"],
    title: "云起东方",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      color: "{colors.primaryDark}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/bamboo-corner.svg",
    body: "竹有节，虚心而直。适合放置人物引语、栏目导语与文化观点。",
    category: "INTRO",
    categoryLabel: "高级模块",
    componentId: "cmp_intro_bamboo_note_002",
    description: "墨竹角花与温润纸色构成的东方引言卡，文字仍可直接编辑。",
    eyebrow: "竹下清谈",
    name: "墨竹人文引言",
    scenarios: ["culture", "interview", "opinion", "poetry"],
    title: "心有清欢，岁月安然",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.border}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/red-gold-wave.svg",
    body: "坚持问题导向和结果导向，以清晰层级呈现工作部署、责任分工与阶段成效。",
    category: "GOV",
    categoryLabel: "高级模块",
    componentId: "cmp_gov_red_gold_banner_001",
    description: "红金飘带 SVG 与正式标题组合，适用于党建、政务、国企与会议报道。",
    eyebrow: "工作部署",
    name: "红金政务主标题",
    scenarios: ["government", "party_building", "meeting", "report"],
    title: "凝心聚力担使命 · 奋楫笃行开新局",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.accent}",
      borderRadius: "{radius.md}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/red-gold-wave.svg",
    body: "请提前安排工作与生活，注意出行安全。具体时间与要求可在这里直接修改。",
    category: "GOV",
    categoryLabel: "高级模块",
    componentId: "cmp_gov_red_gold_notice_002",
    description: "红金边饰的正式通知卡，适合放假通知、会议通知和温馨提醒。",
    eyebrow: "NOTICE · 通知",
    name: "红金正式通知卡",
    scenarios: ["government", "holiday_notice", "meeting", "safety"],
    title: "关于节假日安排的通知",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.primary}",
      ...
      paddingRight: "{spacing.xl}",
      paddingTop: "{spacing.xl}",
      variant: "civic_red_notice",
    },
    variant: "civic_red_notice",
  }),
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/tech-orbit.svg",
    body: "用轨道、网格与金色锚点承载产品价值、关键指标与活动主题。",
    category: "TECH",
    categoryLabel: "高级模块",
    componentId: "cmp_tech_orbit_hero_001",
    description: "原创蓝金科技轨道 SVG 主视觉，适合峰会、产品、科技企业与年度报告。",
    eyebrow: "INNOVATION 2026",
    name: "蓝金科技主视觉",
    scenarios: ["technology", "product", "summit", "report"],
    title: "聚焦新格局 · 拥抱新未来",
    tokenMap: {
      ...
      paddingRight: "{spacing.xl}",
      paddingTop: "{spacing.xl}",
      variant: "tech_orbit_hero",
    },
    variant: "tech_orbit_hero",
  }),
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/leaf-divider.svg",
    body: "柔和的枝叶与稳定留白适合活动回顾、自然教育、公益与生活方式文章。",
    category: "SVG",
    categoryLabel: "SVG装饰",
    componentId: "cmp_intro_leaf_story_003",
    description: "原创森系枝叶 SVG 与轻导语组合，让长文自然分段。",
    eyebrow: "NATURAL MOMENT",
    name: "森系枝叶导语",
    scenarios: ["nature", "summer", "event", "lifestyle"],
    title: "把日子过成喜欢的样子",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderRadius: "{radius.md}",
      ...
      paddingTop: "{spacing.xl}",
      textAlign: "center",
      variant: "leaf_story_intro",
    },
    variant: "leaf_story_intro",
  }),
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/festival-lanterns.svg",
    body: "愿每份热爱都有回响，每次奔赴都有收获。正文与祝福语可自由替换。",
    category: "HERO",
    categoryLabel: "高级模块",
    componentId: "cmp_hero_festival_lantern_002",
    description: "原创灯笼与金色流线 SVG 节庆模块，适合春节、元旦、开业和周年。",
    eyebrow: "HAPPY NEW YEAR",
    name: "灯笼节庆主视觉",
    scenarios: ["festival", "spring_festival", "new_year", "opening"],
    title: "新岁启新章",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.accent}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/photo-film-frame.svg",
    body: "三张图片组成连续叙事，可分别替换为活动、人物或旅行照片。",
    category: "GALLERY",
    categoryLabel: "图集模块",
    componentId: "cmp_gallery_film_triptych_001",
    description: "胶片上下孔位、三图并列和杂志图注组合；插入后替换三个图片槽即可。",
    eyebrow: "PHOTO STORY",
    imageCount: 3,
    name: "胶片三图图集",
    scenarios: ["gallery", "event", "travel", "photography"],
    title: "这一刻，值得被记住",
    tokenMap: {
      backgroundColor: "{colors.primaryDark}",
      borderRadius: "{radius.md}",
  ...
  visualDefinition({
    assetKind: "svg",
    assetPath: "/visual-assets/photo-film-frame.svg",
    body: "一张主图讲场景，一张辅图补充细节，适合人物、产品与活动故事。",
    category: "GALLERY",
    categoryLabel: "图集模块",
    componentId: "cmp_gallery_magazine_duo_002",
    description: "杂志式大小双图结构，保留标题、导语和两个可替换图片槽。",
    eyebrow: "VISUAL JOURNAL",
    imageCount: 2,
    name: "杂志大小双图",
    scenarios: ["gallery", "portrait", "product", "story"],
    title: "用两幅画面讲完一个故事",
    tokenMap: {
      backgroundColor: "{colors.surface}",
      borderColor: "{colors.borderStrong}",
